...
FALLBACK_OUT = BASE / "基于时空图神经网络的多行为序列推荐模型答辩ppt_v4.pptx"
...
def background(slide, index):
    ...
    textbox(slide, f"{index}/9", 12.0, 6.95, 0.8, 0.22, 10, MUTED, False, PP_ALIGN.RIGHT)


def title(slide, text):
    textbox(slide, text, 0.72, 0.30, 6.5, 0.45, 28, DARK, True)
...
def slide1():
    ...
    panel(s, "研究背景", "电商推荐已从静态展示转向智能分发。用户会产生点击、收藏、加购、购买等多种行为，单一交互信号难以完整反映兴趣形成过程。", 0.82, 1.42, 5.45, 1.30, LIGHT)
    ...
    panel(s, "技术路线", "数据清洗与字段统一 → 时空异构图构建 → 模型训练 → 实验分析 → 桌面应用封装。", 0.82, 5.92, 5.45, 1.00, LIGHT)
...
def slide2():
    ...
    background(s, 2)
    title(s, "理论基础与关键技术")
    textbox(s, "推荐任务、评价指标、序列模型、图模型与多行为建模基础", 0.74, 1.02, 7.5, 0.28, 15, MUTED)
    panel(s, "Top-K任务", "R_u^K = TopK_{i∈I} f(u,i)\n推荐的目标是排序，从候选集合中选出前K个最值得展示的商品。", 0.82, 1.42, 3.95, 1.30, LIGHT)
    panel(s, "评价指标", "HR关注是否命中，NDCG关注命中位置，Recall关注覆盖程度。三者联合评价排序效果。", 4.98, 1.42, 3.95, 1.30, WHITE)
    panel(s, "序列模型", "Attention(Q,K,V)=softmax(QK^T/√d)V\nGRU与Transformer分别代表递推式和注意力式时间建模思路。", 9.14, 1.42, 3.35, 1.30, LIGHT)
    panel(s, "图神经网络", "H^(l+1)=σ(ÂH^(l)W^(l))\n节点表示由自身与邻域共同决定，适合学习用户与商品之间的高阶协同关系。", 0.82, 3.05, 5.10, 1.38, WHITE)
    panel(s, "多行为强度", "ω_(u,i,t)=w_b·exp(-λΔt)\n行为语义强弱和时间新鲜性被同时编码为可学习信号。", 6.15, 3.05, 3.45, 1.38, LIGHT)
    panel(s, "排序学习", "L_BPR = -logσ(y_(u,i+) - y_(u,i-))\n模型优化的是正负样本相对次序，这与Top-K推荐目标一致。", 9.82, 3.05, 2.67, 1.38, WHITE)
    panel(s, "理论支撑关系", "Top-K定义明确了推荐目标，评价指标刻画排序效果，序列模型提供时间建模基础，图模型提供结构传播基础，多行为建模补足行为语义与意图差异表达。", 0.82, 4.82, 11.66, 1.48, LIGHT, 15, 14)
...
def slide3():
    ...
    panel(s, "统一字段", "e=(u,i,b,t)\nu表示用户，i表示商品，b表示行为类型，t表示Unix时间戳。四元组形式是统一输入接口。", 4.80, 1.42, 3.75, 1.25, WHITE)
    panel(s, "清洗目标", "处理缺失值、非法编码、异常时间戳、重复记录和顺序错乱，保证数据完整、一致、可解释。", 8.78, 1.42, 3.75, 1.25, LIGHT)
    ...
    panel(s, "图表含义", "两张分布图共同说明数据具有明显长尾特征：少量高活跃用户和头部商品贡献大量交互，大量用户与商品处于长尾区域，因此模型必须处理稀疏性与热门偏置。", 0.82, 5.55, 11.66, 1.18, WHITE)
...
def slide4():
    ...
    panel(s, "图定义", "G=(V,E,φ,ψ)\nV为节点集合，E为边集合，φ和ψ分别表示节点与边类型映射，说明本文采用带类型的时空异构图。", 0.82, 1.42, 4.05, 1.40, LIGHT)
    panel(s, "时间窗口", "W_k=[t_k, t_k+Δ)\n时间窗口把连续时间切分为局部阶段，用于降低长跨度噪声。", 5.08, 1.42, 3.55, 1.40, WHITE)
    panel(s, "动态图快照", "{G^(1), G^(2), …, G^(T)}\n快照序列表达图随时间演化的状态，为新增行为后的局部更新提供基础。", 8.86, 1.42, 3.62, 1.40, LIGHT)
    ...
    panel(s, "行为强度函数", "ω_(u,i,t)=w_b·exp(-λΔt)\n同时量化行为强弱与时间远近，用于边权、样本权重和融合输入。", 6.85, 3.10, 5.60, 1.12, WHITE)
...
def slide5():
    ...
    panel(s, "空间模块输出", "空间模块输出用户空间表示 h_u^sp 和商品空间表示 h_i^sp，为后续时间建模和最终排序提供结构基础。", 6.10, 5.20, 6.38, 1.35, WHITE)
...
def slide7():
    ...
    panel(s, "预测层", "y_(u,i)=z_u^T h_i^sp\n采用内积形式进行候选打分，表达简洁、计算效率高，适合实时排序。", 0.82, 1.42, 4.10, 1.45, LIGHT)
    ...
    bullets(s, [
        ...
        "复杂度主要由图边规模 |E| 和序列长度 L_s 决定。",
        "验证集以 NDCG 作为早停依据，用于平衡拟合能力与泛化稳定性。"
    ], 0.96, 3.30, 11.2, 2.55, 17)
    panel(s, "训练目标对应关系", "预测层回答“用户对哪个商品更可能感兴趣”，BPR与负采样回答“怎样把真正相关的商品排到更前面”，复杂度控制则保证模型能够运行在交互式应用环境中。", 0.82, 5.95, 11.66, 0.78, WHITE)
...
def slide8():
    ...
    panel(s, "模块调用", "整体调用链遵循“界面触发 → 服务调度 → 引擎执行 → 结果回传”。这种设计减少了界面与模型之间的直接耦合。", 6.20, 1.42, 6.28, 1.45, WHITE)
    ...
    panel(s, "引擎实现要点", "推荐引擎统一处理训练、推理、增量更新和历史淘汰。离线路径负责初始训练与周期性重训，在线路径负责用户新增行为后的局部状态刷新，两条路径共享相同的打分与排序逻辑。", 0.82, 6.12, 11.66, 0.70, WHITE)
...
def slide9():
    ...
    panel(s, "系统实现结果", "模型被组织进可运行的软件框架：数据能接入、推荐能刷新、结果能诊断、系统能发布。这使论文工作从算法验证进一步走向了应用落地。", 0.82, 5.40, 11.66, 1.18, WHITE)
